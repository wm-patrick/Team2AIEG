...
def study_mode(current_state: str, minutes: int) -> tuple[str, int, int]:
    """Determines a suggested study mode based on the user's current energy level and mintues available."""

    """ Returns: (descrption_string, work_min, break_min)"""
    ...
    if current_state == "exhausted":
        ...
        if minutes >= 90:
            ...
            return ("Basic study (30 min work / 10 min break, 1 cycles)" 30, 10, )         
    
    elif current_state == "overwhelmed":
        logging.info("Suggestion: Easy material/Flashcards recommended.")
        return "Easy material (15 ,in work / 5 min break, 2 cycle)", 15, 5)

    else:
        logging.info("Suggestion: Basic check-in recommended.")
        return ("Basic check-in (25 min work/ 5 min break, 2 cycle)" 15, 5)
